...
> v2.0.4 定版后的下一轮开发（**分批发**）：补齐原版功能基线。
> ✅ 已完成：**标签页管理**（新建 / 新建列表标签页 / 重命名 / 删除 —— 此前 WinUI 线
> **没有任何新建标签页的入口**）+ **重置数据** + **快捷键体系**（17 条 + 「快捷键参考」窗口）。
> ⏳ 剩余计划：标签页**拖动重排**（留待以后单独一批）、主题引擎（5 预置 + 8 参数细调 +
> 逐令牌覆盖，计划为 v2.0.6）、好用度（批量导入 / 频次排序 / 备份轮转，计划为 v2.0.7）。
